...
defineFeature(feature, (test) => {
  ...
  test("Get directory for empty class", ({ given, when, then, and }) => {
    ...
    when(/^I request the directory for "(.*)"$/, async (className) => {
      ...
      context.response = await request.get(
        `/classes/${classData.id}/directory/json`,
      );
  ...
  test("Get directory with professors only", ({ given, and, when, then }) => {
    ...
    and(
      /^the class has professor "(.*)" with email "(.*)"$/,
      async (professorName, email) => {
  ...
  test("Get directory with TAs and tutors", ({ given, and, when, then }) => {
    ...
    and(
      /^the class has tutor "(.*)" with email "(.*)"$/,
      async (tutorName, email) => {
  ...
  test("Get directory with grouped students", ({ given, when, then, and }) => {
    ...
    and(
      /^the class has student "(.*)" with email "(.*)"$/,
      async (studentName, email) => {
    ...
    and(
      /^the class has another student "(.*)" with email "(.*)"$/,
      async (studentName, email) => {
        await createUserWithDetails({ name: studentName, email });
        await addUserToClass(studentName, "Group Class", "STUDENT");
      },
    ...
    and(
      /^the group "(.*)" should have (\d+) members$/,
      (groupName, memberCount) => {
        const group = context.directory.groups.find(
          (g) => g.name === groupName,
        );
  ...
  test("Get directory with ungrouped students", ({
    given,
    and,
    when,
    then,
    ...
    then(/^the directory should contain (\d+) ungrouped student$/, (count) => {
      expect(context.directory.studentsWithoutGroup).toHaveLength(
        parseInt(count),
      );
    });
  ...
  test("Get directory with complete structure", ({
    given,
    and,
    when,
    then,
    ...
    and(
      /^the class has a third student "(.*)" with email "(.*)"$/,
      async (studentName, email) => {
        await createUserWithDetails({ name: studentName, email });
        await addUserToClass(studentName, "Full Class", "STUDENT");
      },
    ...
    and(/^the total member count should be (\d+)$/, (totalCount) => {
      const actualTotal =
        ...
        context.directory.groups.reduce(
  ...
  test("Verify member details in directory", ({ given, and, when, then }) => {
    ...
    and(
      /^the class has student "(.*)" with the following details:$/,
      async (studentName) => {
    ...
    then(
      /^the student details should include preferred name "(.*)"$/,
      (preferredName) => {
    ...
    and(
      /^the student details should include pronunciation "(.*)"$/,
      (pronunciation) => {
        const student = context.directory.studentsWithoutGroup[0];
  ...
  test("Verify group details in directory", ({ given, and, when, then }) => {
    ...
    and(
      /^there is a group named "(.*)" with the following details:$/,
      async (groupName) => {
